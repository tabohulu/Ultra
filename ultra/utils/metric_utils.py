...
#     """
#     is_valid = torch.tensor(value=is_valid)
#     is_valid.get_shape().assert_has_rank(2)
#     output_shape = is_valid.size()
#
#     if shuffle:
#         values = torch.rand(output_shape, seed=seed)
#     else:
#         values = (
#             torch.ones_like(is_valid, torch.float32) * tf.reverse(
#                 torch.arange(output_shape[1]).type( dtype=tf.float32), [-1]))
#
#     rand = tf.compat.v1.where(
#         is_valid, values, tf.ones(output_shape) * -1e-6)
#     # shape(indices) = [batch_size, list_size]
#     indices = torch.argsort(rand, direction='DESCENDING', stable=True)
...
def gather_nd(params,indices):
    ...
    Args:
      ...
      indices: a K-dimensional integer tensor, best thought of as a (K-1)-dimensional tensor of indices into params,
